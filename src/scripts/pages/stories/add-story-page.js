...
class AddStoryPage {
  ...
  async render() {
    return `
      <section class="container">
        <h1 class="app-title">Tambah Cerita Baru</h1>
  ...
  async _initializeCamera() {
    ...
    if (captureButton) {
      captureButton.addEventListener("click", () => {
        ...
        // Calculate size to maintain aspect ratio but ensure file size < 1MB
        const aspectRatio = video.videoWidth / video.videoHeight;
        let targetWidth = 1280;
        let targetHeight = targetWidth / aspectRatio;

        // Adjust dimensions to ensure reasonable file size
        if (targetHeight > 720) {
          targetHeight = 720;
  ...
  async _createOptimizedPhoto() {
    ...
    if (blob.size > MAX_FILE_SIZE) {
      throw new Error(
        "Tidak dapat mengoptimalkan foto ke ukuran yang diizinkan (max 1MB)"
      );
    }
